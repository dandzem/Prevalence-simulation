...
[
  ...
  vaccinated?         ;; If truem the person is vaccinated
  ...
  nb-dead             ;; Number of dead people at the end of the tick
  nb-vaccinated       ;; Number of vaccinated people at the end of the tick
...
to setup-patches
  ask n-of initial-vaccination-center patches [set pcolor yellow ]  ;; creating n of patches that are vaccination center
end
      ...
      if nearby-uninfected != nobody
       [
        ask nearby-uninfected
         [
           ifelse mutations?  and random-float 100 < mutation-chance                        ;; If mutation switch is on infection chances increase
            [
           ifelse vaccinated?
            [
              if chance-infected-if-vaccinated-mutated < infection-chance-if-mutated     ;;(DONE) ;; NOT DONE YET! If mutation -> increase chance-infected-if-vaccinated AND infection-chance for vaccinated
               [
                set infected? true
            ...
            [
              if random-float 100 < infection-chance-if-mutated                            ;;(DONE) ;; NOT DONE YET! If mutation -> increase infection-chance (DONE)
               [
                set infected? true
                set nb-infected (nb-infected + 1)
               ]
         ...
         [                       ;; If mutation switch is off, infection chances stay the same
          ifelse vaccinated?
           [
             if random-float chance-infected-if-vaccinated < infection-chance     ;; Chance to become infected is changed for vaccinated
             [
               set infected? true
               set nb-infected (nb-infected + 1)
             ]
...
to get-vaccine
  if pcolor = yellow and infected? = false        ;; If person is on patch of vaccination center and not infected
    [
     if random-float 100 < vaccination-chance
      [   ;; with random chance
       set vaccinated? true
      ]                                           ;; become vaccinated
    ]
...
to maybe-recover
  set infection-length infection-length + 1
   ifelse mutations? and random-float 100 < mutation-chance
    [
      if infection-length > recovery-time-mutated        ;; DONE ;; Если мутация увеличить время востановления от болезни(recovery-time) и уменьшить шанс восстановления (recovery-chance)
  [
    ifelse random-float 100 < recovery-chance-mutated
    [
      set infected? false
      set cured? true
    ...
    [
      ...
      die                              ;; else agent dies  and goes +1 to stat
    ...
    [
                                                   ...
                                                   ;; then there is a chance for recovery else die
...
"% vaccinated " 1.0 0 -13791810 true "" "plot ((count turtles with [ vaccinated? ] / initial-people ) * 100 )"
"% dead " 1.0 0 -955883 true "" "plot (( number-dead / initial-people) * 100)"
...
485
158
chance-infected-if-vaccinated-mutated
chance-infected-if-vaccinated-mutated
0
100
44.0
...
@#$#@#$#@
<experiments>
  <experiment name="Virus Only Polio" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ infected? ]</metric>
    <metric>count turtles with [ cured? ] + count turtles with [ infected? ]</metric>
    ...
    <enumeratedValueSet variable="initial-vaccination-center">
      <value value="0"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="Polio virus + Vacination" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ infected? ]</metric>
    <metric>count turtles with [ cured? ] + count turtles with [ infected? ]</metric>
  ...
  <experiment name="Polio virus + mutation" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ infected? ]</metric>
    <metric>count turtles with [ cured? ] + count turtles with [ infected? ]</metric>
  ...
  <experiment name="Polio virus + mutation + vaccination" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ infected? ]</metric>
    <metric>count turtles with [ cured? ] + count turtles with [ infected? ]</metric>
  ...
  <experiment name="Virus only COVID" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ infected? ]</metric>
    <metric>count turtles with [ cured? ] + count turtles with [ infected? ]</metric>
  ...
  <experiment name="COVID virus + Vaccination" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ infected? ]</metric>
    <metric>count turtles with [ cured? ] + count turtles with [ infected? ]</metric>
  ...
  <experiment name="COVID virus + mutation" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ infected? ]</metric>
    <metric>count turtles with [ cured? ] + count turtles with [ infected? ]</metric>
  ...
  <experiment name="COVID virus + mutation + vaccination" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ infected? ]</metric>
    <metric>count turtles with [ cured? ] + count turtles with [ infected? ]</metric>
